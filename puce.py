...
py_ext_path = origin_path+"/Python33/Lib/site-packages"
...
class mainwin(QtGui.QMainWindow):
    ...
    def initUI(self):
        ...
        exitAction = QtGui.QAction(QtGui.QIcon(origin_path+'/images/exit.png'), '&Exit', self)        
        exitAction.setShortcut('Ctrl+Q')
        exitAction.setStatusTip('Exit application')
        exitAction.triggered.connect(QtGui.qApp.quit)
        
        openFile = QtGui.QAction(QtGui.QIcon(origin_path+'/images/openfile.png'), 'Open', self)
        ...
        self.center()
        global vernum
        self.setWindowTitle('PUCE '+str(vernum))  
        self.setWindowIcon(QtGui.QIcon(origin_path+'/images/ico.png'))   
        self.show()
        
    def showDialog(self):
        files = QtGui.QFileDialog.getOpenFileNames(self, 'Open files', origin_path)
...
class mainwidget(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        self.openButton = QtGui.QPushButton("Open files")
        self.applyButton = QtGui.QPushButton("Apply to all")
        label_data = QtGui.QLabel("Raw Data")
        label_data.setAlignment(QtCore.Qt.AlignLeft)
        label_files = QtGui.QLabel("Opened Files")
        label_files.setAlignment(QtCore.Qt.AlignLeft)
        label_lines = QtGui.QLabel("Header Lines")
        label_area = QtGui.QLabel("Cell Area (cm<sup>2</sup>)")
        label_meas = QtGui.QLabel("Measurement Type")
        label_resist = QtGui.QLabel("Resistance Output")
        label_cols = QtGui.QLabel("Columns")
        label_v1 = QtGui.QLabel("V1")
        label_v1.setFixedWidth(300)
        label_v2 = QtGui.QLabel("V2")
        label_v2.setFixedWidth(300)
        label_i1 = QtGui.QLabel("I1")
        label_i1.setFixedWidth(300)
        label_i2 = QtGui.QLabel("I2")
        label_i2.setFixedWidth(300)
        self.line_lines = QtGui.QLineEdit()
        self.line_lines.setFixedWidth(300)
        self.line_area = QtGui.QLineEdit()
        self.line_area.setFixedWidth(300)
        self.line_v1 = QtGui.QLineEdit()
        self.line_v1.setFixedWidth(300)
        self.line_v2 = QtGui.QLineEdit()
        self.line_v2.setFixedWidth(300)
        self.line_v2.setEnabled(False)
        self.line_i1 = QtGui.QLineEdit()
        self.line_i1.setFixedWidth(300)
        self.line_i2 = QtGui.QLineEdit()
        self.line_i2.setFixedWidth(300)
        self.line_i2.setEnabled(False)
        self.combo_meas = QtGui.QComboBox()
        self.combo_meas.setFixedWidth(300)
        self.combo_meas.addItem("Single I/V")
        self.combo_meas.addItem("Double I/V")
        self.combo_resist = QtGui.QComboBox()
        ...
        self.check_delimiter = QtGui.QCheckBox("Other Delimiter than Whitespace")
        self.check_delimiter.setFixedWidth(300)
        self.line_delimiter = QtGui.QLineEdit()
        self.line_delimiter.setFixedWidth(300)
        self.line_delimiter.setEnabled(False)
        self.check_valsur = QtGui.QCheckBox("Values Surrounded")
        self.check_valsur.setFixedWidth(300)
        self.line_valsur = QtGui.QLineEdit()
    ...
    def applytoall(self):
        try:
            ...
            altertbox = QtGui.QMessageBox.warning(self, '','Error occured, check if inputted values are valid.')
    def sync_line_area(self):
        if self.mainfilelist.currentItem():
            try:
                self.mainfilelist.currentItem().cellarea = float(self.line_area.text())
            except:
                altertbox = QtGui.QMessageBox.warning(self, '','Input must be a float')
    def sync_line_lines(self):
        if self.mainfilelist.currentItem():
            try:
                self.mainfilelist.currentItem().headerLines = int(self.line_lines.text())
            except:
                altertbox = QtGui.QMessageBox.warning(self, '','Input must be an integer')
    ...
    def sync_line_valsur(self):
        if self.mainfilelist.currentItem():
            if len(str(self.line_valsur.text())) <= 2:
                ...
                altertbox = QtGui.QMessageBox.warning(self, '','Please input only two characters.')
    def exportToFile(self):
        global vernum
        savelocation = QtGui.QFileDialog.getSaveFileName(self, 'Save File', origin_path+'/output', 'Text file (*.txt)')
        if savelocation:
            with open(savelocation,'w',encoding='utf-8') as outPutFileHeaderLines:
                outPutFileHeaderLines.write('Evaluated using PUCE Version: '+str(vernum)+'\n'+'Evaluation Parameters'+'\n'+'File'+'\t'+'Header lines'+'\t'+'Other delimiter checked'+'\t'+'Other delimiter value'+'\t'+'Values surrounded checked'+'\t'+'Values surrounded value'+'\t'+'Measurement type'+'\t'+'Resistance output'+'\t'+'V1'+'\t'+'I1'+'\t'+'V2'+'\t'+'I2'+'\n')
